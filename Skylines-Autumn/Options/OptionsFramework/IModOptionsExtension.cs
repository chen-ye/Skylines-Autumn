...
namespace DynamicFoliage.OptionsSpace.OptionsFramework
{
    public static class IModOptionsExtension
    {
        ...
        public static bool IsDynamicDropdown<T>(this T value, string propertyName) where T : IModOptions
        {
            var fi = value.GetType().GetProperty(propertyName);
            var attributes = (DynamicDropDownAttribute[])fi.GetCustomAttributes(typeof(DynamicDropDownAttribute), false);
            return attributes.Length > 0;
        }

        public static Action<string> GetDynamicDropdownAction<T>(this T value, string propertyName) where T : IModOptions
        {
            var fi = value.GetType().GetProperty(propertyName);
            var attributes = (DynamicDropDownAttribute[])fi.GetCustomAttributes(typeof(DynamicDropDownAttribute), false);
            if (attributes.Length != 1 || attributes[0].Action == null)
        ...
        public static IList<string> GetDynamicDropdownItems<T>(this T value, string propertyName) where T : IModOptions
        {
            var fi = value.GetType().GetProperty(propertyName);
            var attributes = (DynamicDropDownAttribute[])fi.GetCustomAttributes(typeof(DynamicDropDownAttribute), false);
            if (attributes.Length != 1 || attributes[0].Items == null)
